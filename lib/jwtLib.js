...
// Pattern: Configuration Guard - application authentication always requires an explicit audience.
function requireApplicationAudience(expectedAudience) {
  if (typeof expectedAudience !== 'string' || expectedAudience.trim().length === 0) {
    throw new TypeError('Application JWT verification requires an expected audience');
  }
}

// Pattern: Policy - one application credential belongs to exactly one application and account.
function hasApplicationAudience(payload, expectedAudience) {
  requireApplicationAudience(expectedAudience);
  return typeof payload?.aud === 'string' && payload.aud === expectedAudience;
}

function isApplicationPayload(payload, expectedAudience) {
  return (
    typeof payload?.sub === 'string' &&
    payload.sub.trim().length > 0 &&
    hasApplicationAudience(payload, expectedAudience)
  );
}

// Pattern: Adapter - exposes the existing signature/lifetime reader to non-Express consumers.
function readApplicationJwt(token, verificationJwks, expectedAudience) {
  requireApplicationAudience(expectedAudience);
  const request = { get: () => `Bearer ${token}`, jwt: null };
  if (!tryValidateAndExtractJwtObject(request, verificationJwks)) {
    return null;
  }
  return isApplicationPayload(request.jwt.payload, expectedAudience) ? request.jwt : null;
}

// Pattern: Authentication Boundary - attaches only a token for the configured application.
function validateAndExtractApplicationJwt(
  req,
  verificationJwks,
  expectedAudience,
  customErrorFunction,
  ...
  req.jwt = null;
  requireApplicationAudience(expectedAudience);
  const token = _validateJwt(req, customErrorFunction);
  req.jwt = readApplicationJwt(token, verificationJwks, expectedAudience);
  if (!req.jwt) {
...
// Pattern: Authentication Boundary - validates opaque ownership before attaching existing session claims.
function attachApplicationServerAuthClaims(req, claims, expectedAudience, customErrorFunction) {
  const payload = createServerAuthPayload(claims);
  payload.aud = claims.aud;
  if (!isApplicationPayload(payload, expectedAudience) || isJwtExpired({ jwt: { payload } })) {
    throwError(customErrorFunction);
  }
  attachServerAuthClaims(req, claims, customErrorFunction);
  req.jwt.payload.aud = payload.aud;
}

// Pattern: Credential Dispatch - JWT failures cannot be retried as opaque authentication.
async function validateAndExtractApplicationJwtOrServerAuth(
  req,
  verificationJwks,
  expectedAudience,
  ...
  req.jwt = null;
  requireApplicationAudience(expectedAudience);
  const token = _validateServerAuthToken(req, customErrorFunction);
  if (token.includes('.')) {
    return validateAndExtractApplicationJwt(
      req,
      verificationJwks,
      expectedAudience,
      customErrorFunction,
  ...
  attachApplicationServerAuthClaims(req, claims, expectedAudience, customErrorFunction);
  return req;
}

// Pattern: Middleware - applies the application JWT contract using the existing Express error boundary.
function verifyApplicationJwt(verificationJwks, expectedAudience, customErrorFunction) {
  requireApplicationAudience(expectedAudience);
  return function (req, res, next) {
    try {
      validateAndExtractApplicationJwt(
        req,
        verificationJwks,
        expectedAudience,
        customErrorFunction,
...
// Pattern: Middleware - accepts application-bound JWT or opaque credentials.
function verifyApplicationJwtOrServerAuth(
  verificationJwks,
  expectedAudience,
  serverAuthIntrospector,
  customErrorFunction,
  ...
  return async function (req, res, next) {
    try {
      await validateAndExtractApplicationJwtOrServerAuth(
        req,
        verificationJwks,
        expectedAudience,
        serverAuthIntrospector,
...
// Pattern: Middleware - composes application authentication with existing role semantics.
function verifyApplicationJwtOrServerAuthAndHasRole(
  role,
  verificationJwks,
  expectedAudience,
  serverAuthIntrospector,
  ...
  return async function (req, res, next) {
    try {
      ...
      _isLoginRequired(doesJwtUserHasRole(req, role), customErrorFunction);
      next();
    } catch (error) {
      next(error);
    }
...
module.exports = {
  ...
  readApplicationJwt,
  validateAndExtractApplicationJwt,
  validateAndExtractApplicationJwtOrServerAuth,
  verifyApplicationJwt,
  verifyApplicationJwtOrServerAuth,
  verifyApplicationJwtOrServerAuthAndHasRole,
